...
wine <- read.csv("/Users/watseob/Desktop/DataScience/GitHub/DataScience/Project/P_Wine/wineQualityReds.csv")
str(wine)
dim(wine)
summary(wine)
```
...
## Pair plot
```{r echo=FALSE, pairplot}
set.seed(1234)
sub_wine <- wine[,c(2:13)]
...
```{r echo=FALSE, scatterplor4}
ggplot(wine, aes(x = total.sulfur.dioxide, y = free.sulfur.dioxide)) +
  geom_point() +
  geom_smooth(method = "lm")
summary(lm(wine, formula = free.sulfur.dioxide ~ total.sulfur.dioxide))
...
### volatile acidity, citric acid, quality, level of quality, alcohol

```{r echo=FALSE, Multivariate_Plots3}
wine$fquality <- as.factor(wine$quality)
ggplot(wine,(aes(x=volatile.acidity, y = citric.acid, size = quality, col = alcohol ))) +
  geom_jitter(alpha = 0.6) +
  scale_size(range = c(1,4)) +
  scale_color_continuous(low = "#ffff96", high = "#ff6600") + 
  facet_grid(~level)
```

We can see fixed acidity and citric acid are positively correlated.  
As level of alcohol is higher, quality level is higher. There is a variance of alcohol in the middle quality.
...
As creating violin plots with box plot, we can see density of each y variable.
Lines between median of each quality show us relation with two variable.
